        ...
        int batchsize = fb_h.size()/nbatches;
            for (int epoch=0; epoch<nepoch; epoch++)
            {
                ...
                */
                ...
                FILE* f2 = fopen((data+"_"+version+"/relation2vec."+version).c_str(),"w");
                FILE* f3 = fopen((data+"_"+version+"/entity2vec."+version).c_str(),"w");
...
void prepare()
{
    FILE* f1 = fopen((path+"/entity2id.txt").c_str(),"r");
    FILE* f2 = fopen((path+"/relation2id.txt").c_str(),"r");
    int x;
    ...
    FILE* f_kb = fopen((path+"/train.txt").c_str(),"r");
    while (fscanf(f_kb,"%s",buf)==1)
    {
        ...
        //relation -> s2 entity->s1 or s3 in FB13 and WN11
        ...
        left_entity[relation2id[s2]][entity2id[s1]]++;
        right_entity[relation2id[s2]][entity2id[s3]]++;
        train.add(entity2id[s1],entity2id[s3],relation2id[s1]);
    }
    for (int i=0; i<relation_num; i++)
    {
