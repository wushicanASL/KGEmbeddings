...
def score():
    ...
    score = T.sum( T.abs_(res),axis=1)
    return theano.function(inputs = [idx, ent_embed, rel_embed],\
                           outputs = score,on_unused_input='ignore')
    #theano.function(inputs = [idx, ent_embed, rel_embed , A_h_mat, A_t_mat],\
                           #outputs = score,on_unused_input='ignore')
...
#    print i
entity2vec = np.loadtxt('../'+path+'/entity2vec.bern')
relation2vec = np.loadtxt('../'+path+'/relation2vec.bern')
   # A_h = np.loadtxt('../TranSparse/A_h.bern'+str(i)).reshape((n_relation,20,20))
   # A_t = np.loadtxt('../TranSparse/A_t.bern'+str(i)).reshape((n_relation,20,20))
   ###for transE#
dev_score = score_f(dev_triples_idx, entity2vec, relation2vec)
test_score = score_f(test_triples_idx, entity2vec, relation2vec)
   ##for tranSparse
