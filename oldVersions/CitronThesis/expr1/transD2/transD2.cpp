...
float *entityTransVec, *entityTransVecDao, *relationTransHeadVec, *relationTransHeadVecDao, *relationTransTailVec, *relationTransTailVecDao;
float *tmpValue;

void norm(int h, int t, int r, int j, bool flag) {
		...
		norm(entityVecDao + dimension * h, entityTransVecDao + dimension * h, relationTransHeadVecDao + dimensionR * r);
		norm(entityVecDao + dimension * t, entityTransVecDao + dimension * t, relationTransTailVecDao + dimensionR * r);
                if (flag)
		    norm(entityVecDao + dimension * j, entityTransVecDao + dimension * j, relationTransHeadVecDao + dimensionR * r);
                else
		    norm(entityVecDao + dimension * j, entityTransVecDao + dimension * j, relationTransTailVecDao + dimensionR * r);
...
void init() {
	...
	relationVec = (float *)calloc(relationTotal * dimensionR * 2 + entityTotal * dimension * 2 + relationTotal * dimensionR * 4 + entityTotal * dimension * 2, sizeof(float));
	...
	relationTransHeadVec = entityVecDao + entityTotal * dimension;
	relationTransHeadVecDao = relationTransHeadVec + relationTotal * dimensionR;
        relationTransTailVec = relationTransHeadVecDao + relationTotal * dimensionR;
        relationTransTailVecDao = relationTransTailVec + relationTotal * dimensionR;
	entityTransVec = relationTransTailVecDao + relationTotal * dimensionR;
	...
	for (int i = 0; i < relationTotal; i++) {
		for (int ii=0; ii < dimensionR; ii++) {
			...
			relationTransHeadVec[i * dimensionR + ii] = randn(0, 1.0 / dimensionR, -6 / sqrt(dimensionR), 6 / sqrt(dimensionR));
			relationTransTailVec[i * dimensionR + ii] = randn(0, 1.0 / dimensionR, -6 / sqrt(dimensionR), 6 / sqrt(dimensionR));
		}
		norm(relationTransHeadVec + i * dimensionR, dimensionR);
		norm(relationTransTailVec + i * dimensionR, dimensionR);
	}
...
float calc_sum(int e1, int e2, int rel, float *tmp1, float *tmp2, float &ee1, float &ee2) {
	...
	for (int ii = 0; ii < dimensionR; ii++) {
		tmp1[ii] = ee1 * relationTransHeadVec[lastr + ii];
		tmp2[ii] = ee2 * relationTransTailVec[lastr + ii];
		if (ii < dimension) {
...
void gradient(int e1_a, int e2_a, int rel_a, int belta, int same, float *tmp1, float *tmp2, float &e1, float &e2) {
	...
	for (int ii = 0; ii < dimensionR; ii++) {
		...
		if (x > 0)
			...
			x = -belta * transDAlpha;
		s1 += x * relationTransHeadVec[lastar + ii];
		s2 += x * relationTransTailVec[lastar + ii];
		relationTransHeadVecDao[lastar + ii] -= same * x * e1;
		relationTransTailVecDao[lastar + ii] += same * x * e2;
	...
	s1 = s1 * same;
        s2 = s2 * same;
	for (int ii = 0; ii < dimension; ii++) {
		entityVecDao[lasta1 + ii] -= s1 * entityTransVec[lasta1 + ii];
		entityTransVecDao[lasta1 + ii] -= s1 * entityVec[lasta1 + ii];
		entityVecDao[lasta2 + ii] += s2 * entityTransVec[lasta2 + ii];
		entityTransVecDao[lasta2 + ii] += s2 * entityVec[lasta2 + ii];
	}
}
	...
	for (int k = transDBatch / transDThreads; k >= 0; k--) {
		...
		if (randd(id) % 1000 < pr) {
			...
			train_kb(trainList[i].h, trainList[i].t, trainList[i].r, trainList[i].h, j, trainList[i].r, tmp);
		        norm(trainList[i].h, trainList[i].t, trainList[i].r, j, false);
		} else {
			...
			train_kb(trainList[i].h, trainList[i].t, trainList[i].r, j, trainList[i].t, trainList[i].r, tmp);
		        norm(trainList[i].h, trainList[i].t, trainList[i].r, j, true);
		}
	}
	pthread_exit(NULL);
}
...
void* train_transD(void *con) {
	...
	memcpy(relationTransHeadVecDao, relationTransHeadVec, dimensionR * relationTotal * sizeof(float));
	memcpy(relationTransTailVecDao, relationTransTailVec, dimensionR * relationTotal * sizeof(float));
	...
	for (int epoch = 0; epoch < transDTrainTimes; epoch++) {
		...
		for (int batch = 0; batch < nbatches; batch++) {
			...
			memcpy(relationTransHeadVec, relationTransHeadVecDao, dimensionR * relationTotal * sizeof(float));
			memcpy(relationTransTailVec, relationTransTailVecDao, dimensionR * relationTotal * sizeof(float));
...
void out_transD() {
		...
		for (int i = 0; i < relationTotal; i++) {
			...
			for (int ii = 0; ii < dimensionR; ii++)
				fprintf(f1, "%.6f\t", relationTransHeadVec[last + ii]);
			fprintf(f1,"\n");
		}
                for (int i = 0; i < relationTotal; i++) {
			int last = dimensionR * i;
			for (int ii = 0; ii < dimensionR; ii++)
				fprintf(f1, "%.6f\t", relationTransTailVec[last + ii]);
			fprintf(f1,"\n");
		}
		for (int  i = 0; i < entityTotal; i++) {
			int last = i * dimension;
