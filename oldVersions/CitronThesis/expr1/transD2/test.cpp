...
std::vector< std::vector<double> > etvec, rthvec, rttvec;
...
struct item {
    ...
    item(triple t, bool _f) {
        ...
        double eh = 0, et = 0;
        for (int i = 0; i < D; ++i) {
            eh += etvec[t.h][i] * e2vec[t.h][i];
            et += etvec[t.t][i] * e2vec[t.t][i];
        }
        for (int i = 0; i < D; ++i)
            y += fabs(  (eh * rthvec[t.r][i] + e2vec[t.h][i])
                      + r2vec[t.r][i]
                      - (et * rttvec[t.r][i] + e2vec[t.t][i]));
    }
};
...
int main(int argc, char* argv[]) {
    ...
    e2vec.resize(entnum);
    r2vec.resize(relnum);
    rthvec.resize(relnum);
    rttvec.resize(relnum);
    etvec.resize(entnum);
