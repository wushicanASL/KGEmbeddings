...
std::vector< std::vector<double> > e2vec, r2vec;
...
struct item {
    ...
    item(triple t, bool _f) {
        ...
        y = 0;
        for (int i = 0; i < D; ++i)
            y += fabs(e2vec[t.h][i] + r2vec[t.r][i] - e2vec[t.t][i]);
    }
};
...
int main(int argc, char* argv[]) {
    ...
    std::vector<triple> testset;
    fin.open("./test2id.txt");
    ...
    char number[5];
    for (int ep = 1; ep <= 50; ep += 1) {
        sprintf(number, "%d", ep);

        fin.open(std::string("./entity2vec.vec") + std::string(argv[1]) + std::string(number));
        ...
        fin.open(std::string("./relation2vec.vec") + std::string(argv[1]) + std::string(number));
